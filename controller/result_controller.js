...
exports.getProductDetails = async (req, res)=>{
    ...
    const {quantity,payment,billing_to} = req.query
    const find = await moduleSchema.find({product:product})
    if(find){
...
// });
    ...
    let myprice = parseInt(result.myprice/result.quantity) *parseInt(quantity)
    console.log(myprice,"myprice")
    let pricewithgst = parseInt(myprice*5/100+myprice);
    ...
    billSchema(req.query).save({data})
    res.send({data})
}
else{
